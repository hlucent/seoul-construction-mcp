...
## 민감정보 처리 원칙
- 실명, 전화번호, 소속·직장명은 파일이나 커밋 메시지에 쓰지 않는다. 이 프로젝트를
  누가, 어느 조직에서 만들고 있는지 짐작하게 하는 표현(특정 업종·직군에서만
  쓰는 용어 포함)도 마찬가지로 쓰지 않는다.
- 단, API가 반환하는 데이터 안의 필드명(예: "시공사/감리사/발주처 담당자")은
  서울시가 공개한 데이터 구조를 그대로 옮겨 적는 것이므로 예외다.
- `SEOUL_OPENAPI_KEY` 값 자체도 커밋·로그·문서에 노출하지 말 것 — 항상 플레이스홀더로만 표기.
- **CLAUDE.md, DEVLOG.md, README.md를 새로 쓰거나 고칠 때마다, 커밋하기 전에
  위 기준으로 그 파일 전체를 한 번 다시 읽는다.** 코드 파일보다 이 세 문서에서
  실수가 훨씬 잦다 — 작업 배경을 설명하려다 보면 자연스럽게 구체적인 얘기를
  적게 되기 때문이다. `grep`으로 실명·전화번호 패턴을 검색해보는 것도 방법이다.
